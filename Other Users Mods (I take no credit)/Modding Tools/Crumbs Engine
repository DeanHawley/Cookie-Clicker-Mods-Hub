...
const Crumbs_Init_On_Load = function() {
    ...
    Crumbs.h.resolveInjects = function() {
        if (!Crumbs.h.CSSInjects.length) {
            return;
        }
        let str = '';
        for (let i in Crumbs.h.CSSInjects) {
            str += Crumbs.h.CSSInjects[i] + '\n';
        }
    ...
    Crumbs.h.inRect = function(x, y, t) {
        let c = Math.cos(-t.r);
        let s = Math.sin(-t.r);
        let a = x * c - y * s;
        let b = x * s + y * c;
        return (a > -t.x && a < t.w - t.x && b > -t.y && b < t.h - t.y);
    }
    ...
    Crumbs.h.inOval = function(x, y, rx, ry, ox, oy, r) {
        let c = Math.cos(-r);
        let s = Math.sin(-r);
        let a = x * c - y * s + ox;
        let b = x * s + y * c + oy;
    ...
    Crumbs.h.blend = function(type, data1, data2) {
        if (data1.width != data2.width || data1.height != data2.height) {
            throw 'Width and height not matching!';
        }
    ...
    Crumbs.h.gaussian = function(arr, factor, abyss) {
        if (!Array.isArray(arr)) {
            throw 'Argument "arr" is not an array!';
        }
        if (typeof factor !== 'number') {
            throw 'Argument "factor" should be a number, when it is in fact ' + (typeof factor) + '!';
        }
    ...
    Crumbs.h.gaussianBlurColor = function(imageData, factor, abyss) {
        ...
        let r = new Uint8ClampedArray(data.length / 4),
            g = new Uint8ClampedArray(data.length / 4),
            b = new Uint8ClampedArray(data.length / 4),
            a = new Uint8ClampedArray(data.length / 4);
    ...
    Crumbs.h.fillPattern = function(ctx, img, w, h, dx, dy, dw, dh, sx, sy, sw, sh, offx, offy) {
        if (img.alt != 'blank') {
            ...
            if (offx < 0) {
                offx -= Math.floor(offx / dw) * dw;
            } else if (offx > 0) {
                offx = (offx % dw) - dw;
            }
            if (offy < 0) {
                offy -= Math.floor(offy / dh) * dh;
            } else if (offy > 0) {
                offy = (offy % dh) - dh;
            }
    ...
    Crumbs.canvas.prototype.addShader = function(shader, index) {
        if (index < 0) {
            throw 'Index must be 0 or a positive number.';
        }
    ...
    new Crumbs.canvas(l('sectionLeft'), 'left', 'leftCanvas', 'position: absolute; top: 0; left: 0; z-index: 5;');
    l('backgroundLeftCanvas').style.display = 'none';
    ...
    Crumbs.object = function(obj, parent) {
        ...
        if (typeof obj !== 'object') {
            throw 'Crumbs.object constructor parameter must be an object or undefined.';
        }
    ...
    Crumbs.object.prototype.setAnchor = function(anchor) {
        ...
        if (typeof this.anchor === 'string') {
            throw '"' + anchor + '" is not a valid anchor!"';
        }
    }
    ...
    Crumbs.object.prototype.set = function(o) {
        for (let i in o) {
            if (!Crumbs.nonQuickSettable.includes(i) && !Crumbs.nonValidProperties.includes(i)) {
                this[i] = o[i];
            } else if (Crumbs.nonValidProperties.includes(i)) {
                throw 'Cannot set particle property "' + i + '"!';
            } else if (!Crumbs.allProperties.includes(i)) {
                throw 'Unrecognized particle property: "' + i + '"!';
            }
        }
        if (o.newChild) {
    ...
    Crumbs.object.prototype.commenceInit = function() {
        ...
        for (let i in this.behaviors) {
            if (this.behaviors[i] instanceof Crumbs.behavior) {
                this.behaviors[i] = new Crumbs.behaviorInstance(this.behaviors[i]);
            } else if (this.behaviors[i] instanceof Crumbs.behaviorInstance) {
                continue;
            } else if (typeof this.behaviors[i] === 'function') {
                const b = new Crumbs.behavior(this.behaviors[i]);
                this.behaviors[i] = new Crumbs.behaviorInstance(b);
            } else {
                throw 'Object behavior must be an instance of Crumbs.behavior, Crumbs.behaviorInstance, or is a function!';
            }
        }

        this.imgs = [].concat(this.imgs);
        for (let i in this.imgs) {
            if (typeof this.imgs === 'function') {
        ...
        if (this.parent === null) {
            ...
            for (let i = 0; i < this.scope.objects.length; i++) {
                if (this.scope.objects[i] === null) {
                    this.index = i;
                    this.scope.objects[i] = this;
                    pushed = true;
                    break;
                }
            }
            if (!pushed) {
                this.index = this.scope.objects.length;
                this.scope.objects.push(this);
            }
        } else {
            let pushed = false;
    ...
    Crumbs.object.prototype.addComponent = function(comp) {
        ...
        if (Crumbs.prefs.warnDuplicateComponents) {
    ...
    Crumbs.object.prototype.addBehavior = function(behavior) {
        if (behavior instanceof Crumbs.behavior) {
            behavior = new Crumbs.behaviorInstance(behavior);
        } else if (typeof behavior === 'function') {
            const b = new Crumbs.behavior(behavior);
            behavior = new Crumbs.behaviorInstance(b);
        } else if (!behavior instanceof Crumbs.behaviorInstance) {
            throw 'Object behavior must be an instance of Crumbs.behavior, Crumbs.behaviorInstance, or is a function!';
        }
    ...
    Crumbs.updateObjects = function() { //called every logic frame
        ...
        for (let i in Crumbs.objects) {
            if (Crumbs.objectsEnabled(i)) {
                for (let ii in Crumbs.objects[i]) {
                    if (Crumbs.objects[i][ii] !== null) {
                        Crumbs.objects[i][ii].updateChildren();
                    }
                }
            }
        }
    ...
    Crumbs.spawnVisible = function(obj, custom) {
        if ((Crumbs.lastUpdate + Crumbs.sleepDetectionBuffer < Date.now() || !Game.visible) && !Crumbs.unfocusedSpawn) {
            return false;
        }
    ...
    Crumbs.component.settings = function(obj) {
        ...
        for (let i in obj) {
            if (!globals.includes(i)) {
                throw '"' + i + '" is not a valid setting for a settings component!';
            }
        }
    ...
    Crumbs.component.tCounter.prototype.enable = function() {
        this.enabled = true;
    };
    Crumbs.component.tCounter.prototype.disable = function() {
        this.enabled = false;
    };
    Crumbs.component.tCounter.prototype.init = function(m) {
        m.tCount = 0;
    };
    Crumbs.component.tCounter.prototype.logic = function(m) {
        if (this.function) {
            m.tCount += this.function.call(m, m);
        } else {
            m.tCount++;
        }
    };
    ...
    Crumbs.component.pointerInteractive.prototype.logic = function(m) {
        ...
        if (b && !this.alwaysInteractable) {
            ...
            for (let i = scope.indexOf(m) + 1; i < scope.length; i++) {
                ...
                if (!comp.getHoverStatus(o, Crumbs.getPWidth(o), Crumbs.getPHeight(o))) {
                    continue;
                }
                b = false;
                break;
        ...
        if (b && !this.hovered) {
            this.hovered = true;
            this.onMouseover.call(m);
        } else if (!b && this.hovered) {
            this.hovered = false;
            this.onMouseout.call(m);
        }
        if (this.hovered) {
            if (!this.click && Crumbs.pointerHold) {
                this.click = true;
                this.onClick.call(m);
            }
            if (this.click && !Crumbs.pointerHold) {
                this.click = false;
                this.onRelease.call(m);
            }
        }
    };
    ...
    Crumbs.component.bloom.prototype.enable = function() {
        this.enabled = true;
    }
    Crumbs.component.bloom.prototype.disable = function() {
        this.enabled = false;
    }
    ...
    Crumbs.component.bloom.prototype.preDraw = function(m) {
        if (Crumbs.t - this.lastUpdate > this.updateRate) {
            this.update(m);
        }
    };
    ...
    Crumbs.component.linearFade.prototype.enable = function() {
        this.enabled = true;
    }
    Crumbs.component.linearFade.prototype.disable = function() {
        this.enabled = false;
    }
    ...
    Crumbs.component.linearFade.prototype.drawVertical = function(m, ctx, pWidth, pHeight) {
        ...
        if (initOffset >= 0) {
            ctx.drawImage(pic, 0, 0, pWidth, initOffset, ox, oy, dx, initOffset * dyM);
        }
        for (let i = 0; i < slicesTotal; i++) {
            ctx.globalAlpha -= alphaStep;
            if (initOffset + i * this.sliceWidth > pHeight) {
                return;
            }
        ...
        if (initOffset + slicesTotal * this.sliceWidth > pHeight || !ctx.globalAlpha) {
            return;
        }
        ctx.drawImage(pic,
            0,
    ...
    Crumbs.component.linearFade.prototype.drawHorizontal = function(m, ctx, pWidth, pHeight) {
        ...
        ctx.globalAlpha = this.initialAlpha ?? m.alpha;
        if (initOffset >= 0) {
            ctx.drawImage(pic, 0, 0, initOffset, pHeight, ox, oy, initOffset * dxM, dy);
        }
        for (let i = 0; i < slicesTotal; i++) {
            ctx.globalAlpha -= alphaStep;
            if (initOffset + i * this.sliceWidth > pWidth) {
                return;
            }
        ...
        ctx.globalAlpha = this.finalAlpha ?? 0;
        if (initOffset + slicesTotal * this.sliceWidth > pWidth || !ctx.globalAlpha) {
            return;
        }
        ctx.drawImage(pic,
            initOffset + slicesTotal * this.sliceWidth,
    ...
    Crumbs.shaderDefaults.allWhite = {
        enabled: true
    }
    ...
    Crumbs.particle = function(obj, x, y, r, a, scope) {
        ...
        for (let i in Crumbs.particleDefaults) {
            this[i] = Crumbs.particleDefaults[i];
        }
        for (let i in obj) {
            if (i != 'img') {
                this[i] = obj[i];
        ...
        if (this.scope) {
            ...
            throw scope + ' is not a valid particle scope!';
        }
    ...
    Crumbs.drawAnchorDisplay = function(o, ctx, p) {
        ctx.save();
        if (o.anchorDisplayColor) {
            ctx.fillStyle = o.anchorDisplayColor;
        } else if (o.parent) {
            ctx.fillStyle = '#57d2f2';
        } else {
            ctx.fillstyle = '#ccfffb';
        }
        ctx.globalAlpha = 1;
    ...
    Crumbs.iterateObject = function(o, ctx) {
        ...
        for (let ii = 0; ii < o.components.length; ii++) {
            if (o.components[ii].enabled) {
                o.components[ii].preDraw(o, ctx);
            }
        }
        ...
        for (let i in toDraw) {
            ...
            if (Crumbs.prefs.anchorDisplay) {
                Crumbs.drawAnchorDisplay(o, ctx);
            }
            for (let ii = o.components.length - 1; ii >= 0; ii--) {
                if (o.components[ii].enabled) {
                    o.components[ii].postDraw(o, ctx);
                }
            }
        }
    ...
    Crumbs.drawObjects = function() {
        for (let c in Crumbs.scopedCanvas) {
            ...
            for (let i in Crumbs.particles[c]) {
                ...
                ctx.translate(-p.x, -p.y);
                if (p.globalCompositeOperation) {
                    ctx.globalCompositeOperation = settingObj.globalCompositeOperation;
                }
            }
            ctx.globalAlpha = 1;
            ...
            if (!Crumbs.scopedCanvas[c].shaders.length) {
                continue;
            }
        ...
        this.waitForLoad = function(assets, callback) {
            ...
            let checkLoadedLoop = function() {
                for (let i = 0; i < assets.length; i++) {
                    if (!me.assetsLoaded.has(assets[i])) {
                        setTimeout(checkLoadedLoop, 200);
                        return false
                    }
                }
                callback();
