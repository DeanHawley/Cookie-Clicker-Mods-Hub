(function() {
            ...
            (function() {
                ...
                Steam.modsPopup = function() {
                    ...
                    function renderModOptions(mod, index) {
                        ...
                        const modDisableEl = l("modDisable");
                        if (modDisableEl) {
                            AddEvent(modDisableEl, "click", () => {
                                Game.ToggleMod(mod.id, !mod.disabled);
                                mod.disabled = !mod.disabled;
                                renderModOptions(mod, index);
                                renderModList(index);
                    ...
                    Game.Prompt(`
                        ...
                        <div style="overflow:hidden;clear:both;">
                            <a class="option" style="float:left;" onclick="Game.OpenFolder('mods');">${loc("Open /mods folder")}</a>
                            <a class="option" style="float:right;" onclick="Steam.workshopPopup();">${loc("Open Workshop")}</a>
                        </div>
                    ...
                    `, [
                        [loc("Restart with new changes"), 'Game.ConfirmRestart();', 'display:none;'], loc("Back")
                    ], 0, 'widePrompt');
                    renderModList();
                };
                ...
                Steam.workshopPopup = function() {
                    Game.Prompt(`
                        ...
                        <div class="block" id="modDisplay">
                            ...
                            <div class="line"></div>
                            <div style="overflow:hidden;clear:both;">
                                <a class="option" style="display:block;font-weight:bold;" onclick="Game.OpenFolder('mods');">${loc("New mod")} - ${loc("Select folder")}</a>
                                <a class="option" style="display:block;font-weight:bold;" onclick="Game.Prompt('This feature is not yet implemented.');">${loc("Update published mods")}...</a>
                            </div>
                        </div>
                    `, [
                ...
                async function j() {
                    try {
                        ...
                        if (typeof Game !== "undefined") {
                            ...
                            waitForElement("heraldsAmount", (o) => {
                                o.textContent = Game.heralds;
                                o.style.fontSize = "12px";
                            });
                        }
                    } catch (e) {
                        console.error("Error fetching heralds:", e);
                    }
                }
                j();
                setInterval(j, 10000);
    ...
    const beam = document.getElementById('rightBeam') || document.querySelector('.separatorRight');
    const middle = document.getElementById('sectionMiddle') || document.getElementById('middleSection') || document.querySelector('#rows');

    if (beam && middle) {
        beam.style.transform = 'translateX(17px)';

        middle.style.width = (middle.getBoundingClientRect().width + 1) + 'px';
    }
})();
