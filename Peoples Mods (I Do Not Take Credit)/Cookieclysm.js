...
function shadowSave(type) {
    ...
    if (type == 2 || type == 3) {
        ...
        if (Game.useLocalStorage) {
            ...
            if (str.length < 10) {} else {
                str = escape(str);
                localStorageSet(Game.SaveTo, str);
                if (App) App.save(str);
                if (!localStorageGet(Game.SaveTo)) {} else if (document.hasFocus()) {}
            }
        } else {
            var now = new Date();
            ...
            document.cookie = str;
            if (App) App.save(str);
            if (document.cookie.indexOf(Game.SaveTo) < 0) {} else if (document.hasFocus()) {}
        }
    }
}
...
C.load = function(str) {
    ...
    let youpocData = spl[7].split(',');
    C.youWrath = parseFloat(youpocData[0]) || 0;
...
Game.GiveUpAscend = function(bypass) {
    if (!bypass) Game.Prompt('<h3>Give up</h3><div class="block">Are you sure? You\'ll have to start this run over and won\'t gain any heavenly chips!</div>', [
        ['Yes', 'Game.ClosePrompt();Game.GiveUpAscend(1);'], 'No'
    ]);
    else {
...
var NewBuilding = function(name, commonName, desc, icon, iconColumn, art, price, cps, buyFunction, foolObject, buildingSpecial) {
    ...
    AddEvent(me.canvas, 'mouseover', function(me) {
        return function() {
            me.mouseOn = true;
        }
    }(me));
    AddEvent(me.canvas, 'mouseout', function(me) {
        return function() {
            me.mouseOn = false;
        }
    }(me));
    AddEvent(me.canvas, 'mousemove', function(me) {
        return function(e) {
...
eval(`Game.BuildStore = ` + Game.BuildStore.toString().replace(`me.l=l('product'+me.id);`, `me.l=l('product'+me.id); Game.Objects['Converter'].l.childNodes[0].style.background = Game.Objects['Converter'].l.childNodes[1].style.background = 'url("https://raw.githubusercontent.com/yeetdragon24/cookieclysm/main/img/converterIcon.png")';`));
...
C.cursorUpgrade = function(name, quote, price, power) {
    let upgrade = new Game.Upgrade(name, `You have <b>${power} more</b> available cursor ring${power==1?'':'s'}.<q>${quote}</q>`, price, [9, C.cursorUpgrades.length, icons]);
    upgrade.power = power;
...
C.clysmUpgrade = function(name, desc, price, icon) {
    ...
    upgrade.showIf = function() {
        return C.cookieclysm;
    };
...
Game.last.order = 10000.44;

order = 10070;
C.upAndAchiev.push(Game.NewUpgradeCookie({
    name: 'Hard Pasta',
    desc: 'You should probably cook these. Or should you bake them?',
    icon: [28, 32],
...
Game.last.parents = [Game.Upgrades['Unshackled idleverses']];
...
C.upAndAchiev.push(new C.clysmUpgrade('Rift', 'Begins the Cookieclysm.<q>The tear in reality created by the anger of your clones.</q>', 300e6, [31, 2]));
Game.last.posX = -830;
Game.last.posY = -628;
Game.last.buyFunction = function() {
    C.cookieclysm = true;
...
C.upAndAchiev.push(new Game.Upgrade('Pantheon background switch', '', 0, [5, 6]));
Game.last.descFunc = function() {
    return `Currently: <b>${C.pantheonBG ? 'Palace' : 'Forest'}</b> ${tinyIcon(C.pantheonBG ? [5, 6] : [16, 0])}`;
};
Game.last.buyFunction = function() {
    C.pantheonBG = Number(!C.pantheonBG);
    Game.Lock(this.name);
    Game.Unlock(this.name);
    l('templeBG').style.backgroundImage = `url('img/shadedBorders.png'), url('${C.pantheonBGs[C.pantheonBG]}')`;
    l('templeBG').style.backgroundSize = C.pantheonBG ? '50%' : '100% 100%, auto';
};
Game.last.iconFunction = function() {
    return C.pantheonBG ? [5, 6] : [16, 0]
};
Game.last.pool = 'toggle';

order = 24000;
C.upAndAchiev.push(new Game.Upgrade('Cataclysmic biscuit', 'Triggers <b>Cookieclysm season</b> for the next 24 hours.<br>Triggering another season will cancel this one.<br>Cost scales with unbuffed CpS and increases with every season switch.<q>please don\'t make me go back</q>', Game.seasonTriggerBasePrice, [0, 0, icons]));
Game.last.season = 'cookieclysm';
Game.last.pool = 'toggle';
Game.Upgrades['Cataclysmic biscuit'].descFunc = function() {
...
order = 19000;
C.upAndAchiev.push(Game.TieredUpgrade('Fortune #021', '<q>Nothing ever stays exactly the same.</q>', 'Converter', 'fortune'));
Game.Tiers['fortune'].upgrades.push(Game.last);

order = 5000;
C.upAndAchiev.push(Game.SynergyUpgrade('Overgrown plants', '<q>Less maintenence required and you can turn the waste into cookies too!</q>', 'Converter', 'Farm', 'synergy1'));
Game.last.icon[2] = icons;
C.upAndAchiev.push(Game.SynergyUpgrade('Clone sacrifice', `</q><br>Converts one of your clones into <b>one tenth of its cost</b> worth of cookies, every 30 seconds.<q>${Math.random() < 0.1 ? 'How bad can this possibly be?' : 'What could go wrong?'}</q>`, 'Converter', 'You', 'synergy2'));
Game.last.icon[2] = icons;
Game.last.buyFunction = function() {
    Game.Notify('Clone sacrifice', `Conversion of your clones into cookies has began following your purchase of <b>Clone Sacrifice</b>.`, [5, 3, icons]);
};
...
Game.last.buildingTie = Game.Objects['Converter'];
...
Game.last.buildingTie = Game.Objects['Converter'];
C.upAndAchiev.push(new Game.Achievement('Slender baryons', '', [5, 26]));
converter.levelAchiev10 = Game.last;
Game.last.ddesc = '';
Game.last.icon[2] = icons;
...
C.spaceUpgrades = [];
C.upAndAchiev.push(C.spaceUpgrade('Shed', 5, '<q>You can\'t live here, but this will be able to shelter quite a few mice.</q>', 500, [7, Game.Tiers[1].iconRow, icons]));
C.upAndAchiev.push(C.spaceUpgrade('House', 2, '<q>A nice house for grandmas to bake cookies in.</q>', 500e2, [7, Game.Tiers[2].iconRow, icons]));
C.upAndAchiev.push(C.spaceUpgrade('Field', 5, '<q>Now you can farm and dig all you want.</q>', 500e4, [7, Game.Tiers[3].iconRow, icons]));
C.upAndAchiev.push(C.spaceUpgrade('Warehouse', 2, '<q>A large empty building, ready to be populated with your cookie-creating machines.</q>', 500e6, [7, Game.Tiers[4].iconRow, icons]));
...
C.upAndAchiev.push(C.spaceUpgrade('Planet', 5, '<q>It\'s about time you got yourself a place large enough to run your business with more privacy.</q>', 500e9, [7, Game.Tiers[5].iconRow, icons]));
C.upAndAchiev.push(C.spaceUpgrade('The future', 2, '<q>Running out of space? Make it a problem for future you instead!</q>', 500e12, [7, Game.Tiers[6].iconRow, icons]));
C.upAndAchiev.push(C.spaceUpgrade('Distant objects', 5, '<q>If very distant objects are moving away from us faster than the speed of light, why not use them to go past the observable universe so you can store more things?</q>', 500e15, [7, Game.Tiers[7].iconRow, icons]));
C.upAndAchiev.push(C.spaceUpgrade('Quantum optimazation', 2, '<q>Almost all of an atom is empty space. Instead of wasting this space, you squeeze down the subatomic particles until cookies are substantially smaller.</q>', 500e18, [7, Game.Tiers[8].iconRow, icons]));
...
Game.PrestigeUpgrades.push(Game.last);
Game.last.pool = 'prestige';
Game.last.parents = [Game.Upgrades['Box of maybe cookies'], Game.Upgrades['Box of not cookies'], Game.Upgrades['Box of pastries']];
Game.last.posX = -667;
Game.last.posY = -1497;
order = 10070;
C.upAndAchiev.push(Game.NewUpgradeCookie({
    name: 'The Running Burrito',
    desc: 'Just plain chips, not anything special.',
    icon: [6, 5, icons],
    require: 'Box of random stuff we found on the ground',
    power: 5,
    price: Math.pow(10, 51)
}));
C.upAndAchiev.push(Game.NewUpgradeCookie({
    name: 'Playing card',
    desc: 'Picking this off the floor makes you feel terrible, but not the worst.',
    icon: [6, 2, icons],
    require: 'Box of random stuff we found on the ground',
    power: 5,
    price: Math.pow(10, 54)
}));
C.upAndAchiev.push(Game.NewUpgradeCookie({
    name: 'Jealousy',
    desc: 'It\'s unclear how you got jealousy in a tangible form, but it definitely has a lot of sugar.',
    icon: [6, 3, icons],
    require: 'Box of random stuff we found on the ground',
    power: 5,
...
C.upAndAchiev.push(Game.NewUpgradeCookie({
    name: 'Pure black and white cookies',
    desc: 'You didn\'t eat any of these. So why is one missing?',
    icon: [0, 4],
    require: 'Box of random stuff we found on the ground',
    power: 5,
    price: Math.pow(10, 60)
}));
C.upAndAchiev.push(Game.NewUpgradeCookie({
    name: 'Snow',
...
C.upAndAchiev.push(Game.NewUpgradeCookie({
    ...
    desc: 'Not legally cheese, likely plastic.',
    icon: [6, 4, icons],
    require: 'Box of random stuff we found on the ground',
    power: 5,
    price: Math.pow(10, 57)
...
Game.last.scaling = 'medium';
Game.last.increment = 3;
tUSA = 12292023; //Game.last.id; //transcendent Upgrades Start At
order = 200;
C.upAndAchiev.push(C.transcendentUpgrade('Transcendent kittens', 'Kittens are <b>%%% ➡ %%%</b> more powerful.<q>We are not meow mortals.</q>', 9, [18, 35], {
    scaling: 'hard',
    increment: 0.5
}));
...
order = 1000;
C.upAndAchiev.push(C.transcendentUpgrade('Eternal engagement', 'Gain <b>+%%% ➡ +%%%</b> offline CpS.<q>You work faster if you\'re being watched.</q>', 14, Game.season == 'cookieclysm' ? [30, 20] : [1, 5, icons], {
    scaling: 'soft',
    increment: 1
}));
C.upAndAchiev.push(C.transcendentUpgrade('Oversweetened rest', 'Your offline CpS gain is increased by <b>+%%% ➡ +%%%</b> per unspent sugar lump, up to 555 sugar lumps.<q>Invest, leave, and come back rich.</q>', 25, [1, 2, icons], {
    scaling: 'medium',
    increment: 0.05
}));
C.upAndAchiev.push(C.transcendentUpgrade('Glimmering hope', 'The shimmering veil gains an extra <b>%%% ➡ %%% chance</b> not to break.<q>Glimmeringue with a chance to escape.</q>', 42, [3, 2, icons], {
    scaling: 'medium',
    increment: 0.5
}));
C.upAndAchiev.push(C.transcendentUpgrade('Dedicated evasion', 'Holobore, Spirit of Asceticism gains a chance to not unslot when clicking a golden cookie.<q>But how long can you evade them for?</q>', 67, [21, 18], {
    scaling: 'soft',
    increment: 0.01
}));
C.upAndAchiev.push(C.transcendentUpgrade('Locked in', 'After no activity for 5 minutes, cookie production rapidly increases up to <b>+%%% ➡ +%%%</b>.<q>Can\'t move. Can\'t talk. Can only blink.</q>', 128, [29, 2], {
    scaling: 'hard',
    increment: 10
}));
C.upAndAchiev.push(C.transcendentUpgrade('A light in the dark', 'Brightens the Shimmering Veil, giving an additional <b>+%%% ➡ +%%%</b> CpS while the Shimmering Veil is active.<q>The light at the end of the tunnel.</q>', 150, [2, 1, icons], {
    scaling: 'medium',
    increment: 2
}));
C.upAndAchiev.push(C.transcendentUpgrade('Multitasking', 'You can stay <b>Locked in</b> while doing everything except clicking the cookie.<q>You can\'t move, yet you are.</q>', 100000, [12, 31]));
Game.last.parents = [Game.Upgrades['Locked in']];
Game.last.notTiered = 1;

C.upAndAchiev.push(C.transcendentUpgrade('Dark momentum', 'Every 1000 clicks grants <b>+%%% ➡ +%%% CpS</b>.<br><div class="warning">Resets on ascension.</div><q>To the moon.</q>', 60, [2, 4, icons], {
    scaling: 'medium',
    increment: 0.1
}));
C.upAndAchiev.push(C.transcendentUpgrade('Emotionless', '<q>Why are we still here? Just to suffer...</q>', 99, [1, 1, icons], {
    scaling: 'hard',
    increment: 0.2
}));
C.upAndAchiev.push(C.transcendentUpgrade('Denser minerals', 'All mouse upgrades are buffed by <b>%%% ➡ %%%</b>.<q>Glossier than the shiniest Iridyum.</q>', 150, [0, 1, icons], {
    scaling: 'hard',
    increment: 0.1
}));
C.upAndAchiev.push(C.transcendentUpgrade('Flexible', 'Gain <b>%%% ➡ %%%</b> <span style="color:#00FFFF">double click chance</span>.<q>Don\'t limit yourself to two fingers.</q>', 100, [2, 3, icons], {
    scaling: 'hard',
    increment: 2
}));
C.upAndAchiev.push(C.transcendentUpgrade('No hesitation', `Reduces the time it takes for a golden cookie to spawn by <b>%%% ➡ %%%</b>.<q>${Math.random() < 0.1 ? 'When you see a chance, you have to take it and run.' : 'Hesitation is defeat.'}</q>`, 17, [0, 6, icons], {
    scaling: 'hard',
    increment: 0.1
}));

C.upAndAchiev.push(C.transcendentUpgrade('Useless treasure', 'Turns that useless silver into white chocolate, granting <b>+%%% ➡ +%%% Alchemy lab CpS</b>.<q>Silver found to also be transmutable into white chocolate!</q>', 3, [6, 30], {
    scaling: 'hard',
    increment: 1000
}));
C.upAndAchiev.push(C.transcendentUpgrade('Efficient mana', 'Casting spells uses <b>%%% ➡ %%% less magic</b>.<q>Casting spells is a breeze to you now.</q>', 4, [0, 4, icons], {
    scaling: 'hard',
    increment: 0.01
}));
C.upAndAchiev.push(C.transcendentUpgrade('Cheese', 'Muridal gains some bonuses.', 40, [0, 3, icons], {
    scaling: 'soft',
    increment: 1
}));
Game.last.descFunc = function() {
    return this.desc + (this.bought ? '<br>These bonuses are increased by <b>+%%% ➡ +%%%</b>.<q>Swiss cheese, to be exact.</q>' : '')
};
C.upAndAchiev.push(C.transcendentUpgrade('Finally, some rest', 'Selebrak gains some bonuses.', 331, [1, 4, icons], {
    scaling: 'soft',
    increment: 1
}));
Game.last.descFunc = function() {
    return this.desc + (this.bought ? '<br>These bonuses are increased by <b>+%%% ➡ +%%%</b>.' : '') + '<q>More festivity, more regret, more cookies.</q>';
};
C.upAndAchiev.push(C.transcendentUpgrade('Fiery storm', 'Wrinklers spawn <b>+%%% ➡ +%%%</b> faster.<q>Within this lapis cookie lies the flames of pure ambition.</q>', 200, [19, 6], {
    scaling: 'medium',
    increment: 66
}));
C.upAndAchiev.push(C.transcendentUpgrade('Mone buff', '', 10, [12, 14], {
    scaling: 'medium',
    increment: 1
}));
Game.last.descFunc = function() {
    this.dname = C.moneName + ' buff';
    return `Gain <b>+%%% ➡ +%%%</b> more ${C.moneName} from Clone worlds.<br>Every 3 tiers <b>increases the digit cap</b> of the amount of ${C.moneName} you can gain in the clone world by 1.<q>To climb the mountain faster.<br>Current digit cap: ${Math.floor(Game.Upgrades['Mone buff'].tier/3) + 2} ${Beautify((10 ** (Math.floor(Game.Upgrades['Mone buff'].tier/3) + 2)) - 1)}</q>`;
};
C.upAndAchiev.push(C.transcendentUpgrade('Puck', 'Skruuia gains some bonuses.', 14, [2, 2, icons], {
    scaling: 'medium',
    increment: 5
}));
Game.last.descFunc = function() {
    return this.desc + (this.bought ? '<br>These bonuses are increased by <b>+%%% ➡ +%%%</b>.' : '') + '<q>Always left at the bottom.</q>';
};
C.upAndAchiev.push(C.transcendentUpgrade('Mutation', 'Mokalsium gains some bonuses.', 10, [26, 1], {
    scaling: 'medium',
    increment: 25
}));
Game.last.descFunc = function() {
    return this.desc + (this.bought ? '<br>These bonuses are increased by <b>+%%% ➡ +%%%</b>.' : '') + '<q>Descending infinitely, into the fiery abyss.</q>';
};
...
C.upAndAchiev.push(Game.NewUpgradeCookie({
    name: 'RMS Carpathia',
    desc: 'The RMS <i>Carpathia</i> was a passenger steamship built between 190&zwnj;2 and 190&zwnj;3. The <i>Carpathia</i> is most famous for saving 705 passengers of the RMS <i>Titanic</i> on April 15th, 191&zwnj;2. Unfortunately, the <i>Carpathia</i> was sunk on July 17th, 191&zwnj;8, during World War I. The <i>Carpathia</i> is named after the Carpathian Mountains, found in central Europe.',
    icon: [0, 4],
    require: 'Box of random stuff we found on the ground',
    power: 5,
    price: Math.pow(10, 48)
}));
C.upAndAchiev.push(C.transcendentUpgrade('Brighter gloves', 'Buffs Aura gloves by <b>+%%% ➡ +%%%</b>.<q>Create a world of light.</q>', 30, [3, 3, icons], {
    scaling: 'soft',
    increment: 0.2
}));
...
C.upAndAchiev.push(C.cursorUpgrade('Mars', 'Are the moons edible?', 5e11, 1));
...
order = 254;
C.upAndAchiev.push(new Game.Upgrade('Ring gloves', 'You have <b>3 more</b> available cursor rings.', 555555555555555, [9, 16, icons]));
Game.last.pool = 'prestige';
Game.PrestigeUpgrades.push(Game.last);
Game.last.posX = -127 - (23 * 1.75);
Game.last.posY = -415 - (216 * 1.75);
Game.last.parents = [Game.Upgrades['Luminous gloves']];
C.upAndAchiev.push(C.transcendentUpgrade('Lord of the Rings', 'Gain <b>+%% ➡ +%%</b> available cursor rings.<q>One Ring to bring them all, and in the darkness click them.</q>', 3, [2, 6, icons], {
    scaling: 'hard',
    increment: 1
}));
Game.last.parents = [Game.Upgrades['Brighter gloves']];

order = 10500;
C.upAndAchiev.push(new Game.Achievement('Spender', '', [3, 0, icons]));
Game.last.descFunc = function() {
    this.dname = 'Sp&eacute;nder';
    return `Spend <b>100</b> ${C.moneName}.`;
};
C.upAndAchiev.push(new Game.Achievement('Bezos', '', [3, 0, icons]));
Game.last.descFunc = function() {
    this.dname = 'B&eacute;zos';
    return `Spend <b>10,000</b> ${C.moneName}.`;
};
Game.last.pool = 'shadow';
...
Game.last.parents = [Game.Upgrades['Rift']];
Game.last.posX = -1125;
Game.last.posY = -724;
...
C.upAndAchiev.push(C.clysmUpgrade('Red ants', '<b>Red ants</b> will be able to appear. Red ants return their stolen cookies automatically.<q>Are they truly friendly, or do they have their own sinister agenda?</q>', 600e6, [8, 1, icons]));
C.upAndAchiev.push(C.clysmUpgrade('Aphids', 'Ants can infest farms. Each ant makes farms <b>5%</b> less efficient.', 600e6, [8, 0, icons]));
C.upAndAchiev.push(C.clysmUpgrade('Antfestation', 'Each ant increases the chances of another ant appearing.<q>Ants can leave chemical trails signaling for others to follow.</q>', 600e6, [8, 0, icons]));
...
C.upAndAchiev.push(C.clysmUpgrade('Oil', 'You are <b>5%</b> less efficient.<br>Mines are <b>five times/b> as efficient.', 600e6, [13, 33]));
C.upAndAchiev.push(C.clysmUpgrade('Rail', 'You are <b>5%</b> less efficient.<br>Shipments are <b>five times</b> as efficient.', 600e6, [25, 21]));
C.upAndAchiev.push(C.clysmUpgrade('Steel', 'You are <b>5%</b> less efficient.<br>Factories are <b>five times</b> as efficient.', 600e6, [33, 12]));
C.upAndAchiev.push(C.clysmUpgrade('Banking', 'Unlock the oil, rail, and steel stocks in the stock market.', 600e6, [15, 17]));
//cheap labor: you are 5% cheaper, angier faster and sacrificed faster

C.upAndAchiev.push(C.clysmUpgrade('Industrialization', 'Factories boost other buildings.', 600e6, [16, 7]));
C.upAndAchiev.push(C.clysmUpgrade('Market expansion', 'Idleverses boost CpS.', 600e6, [11, 33]));
C.upAndAchiev.push(C.clysmUpgrade('Job availability', 'You are <b>25%</b> cheaper.', 600e6, [18, 0]));
C.upAndAchiev.push(C.clysmUpgrade('Dogma of Dough', 'Your cookies in bank is capped to <b>1 day</b> of production.<br>Buildings are 15% cheaper while you are at the cap.', 600e6, [22, 12]));
//something good ot enncourage players to buy Dogma of Dough

C.upAndAchiev.push(C.clysmUpgrade('Bug bites', 'Ants can infest you. Each ant makes you <b>5%</b> less efficient.', 600e6, [8, 0, icons]));
C.upAndAchiev.push(C.clysmUpgrade('Features', 'Ants can infest Javascript consoles. Each ant makes Javascript consoles <b>5%</b> less efficient.', 600e6, [8, 0, icons]));

order = 6001;
C.upAndAchiev.push(new Game.Achievement('Transcendent explorer', 'Buy <b>5</b> tiers of transcendent upgrades.', [10, 3, icons]));
C.upAndAchiev.push(new Game.Achievement('Transcendent warrior', 'Buy <b>55</b> tiers of transcendent upgrades.', [10, 3, icons]));
C.upAndAchiev.push(new Game.Achievement('Transcendent conqueror', 'Buy <b>555</b> tiers of transcendent upgrades.', [10, 3, icons]));
...
C.getMaxBuildingSpace = function() {
    ...
    C.spaceUpgrades.forEach(function(upgrade) {
        if (Game.Has(upgrade.name)) buildingSpace *= upgrade.power;
    });
...
document.querySelectorAll('.productButton.productMute').forEach(function(x) {
...
eval('Game.UpdateMenu = ' + Game.UpdateMenu.toString()
    .replace(`+Game.version+'</div>'+`, `+Game.version+'</div><br>' + C.getSpaceHTML() + '<br><div class="listing"><b>Double click chance: </b>' + Beautify(C.getEffects('Flexible')) + '%</div>'+`)
...
C.getPos = function(posX, posY) {
    return [Game.bounds.width / 2 + (posX * 3), Game.bounds.height / 2 + (posY * 3)];
}
C.setTUPos = function(name, posX, posY) {
    GU[Game.Upgrades[name].id] = C.getPos(posX, posY);
}
    ...
    for (var i = 0; i < slots.length; i++) {
        ...
        Game.Upgrades[slots[i]].activateFunction = function() {
            ...
            Object.values(Game.Objects).forEach(function(me) {
                for (let slot of C.unshackleSlots) {
                    if (slot == me.id) return;
            ...
            Game.Prompt('<id PickUnshackleUpgrade><h3>' + loc("Pick a building to unshackle") + '</h3>' +
                ...
                '<div class="block" style="float:right;width:152px;clear:right;height:234px;">' + loc("Here are all the unshackle upgrades you\'ve bought.<div class=\"line\"></div>Pick one to unshackle it!<div class=\"line\"></div>You can reassign this slot anytime you transcend.") + '</div>', [
                    [loc("Confirm"), 'C.unshackleSlots[' + slot + ']=Game.SelectingPermanentUpgrade;C.buildTranscendTree();Game.ClosePrompt();'], loc("Cancel")
                ], 0, 'widePrompt');
        }
...
C.initTUParents = function() {
    ...
    Game.Upgrades['Fiery storm'].parents = [Game.Upgrades['Cheese']];
...
C.transcendModifierPool = function() {
    return [0, 0, 0, 1];
}

C.transcend = function(bypass) {
    if (!bypass) Game.Prompt('<id Transcend><h3>' + loc("Transcend") + '</h3><div class="block">' + loc("Are you ready to leave everything behind and travel to a higher realm?") + '</div>', [
        [loc("Yes"), 'Game.ClosePrompt();C.transcend(1);'], loc("No")
    ]);
    else {
...
C.leaveTranscend = function(bypass) {
    if (!bypass) {
        ...
        Game.Prompt('<id Descend><h3>Descend</h3><div class="block">Are you ready to return to reality?' + (Math.random() < (C.transcendModifier != 0 ? 0.8 : 0.3) ? '<div class="line"></div><span class="red">Things might be a little different than what you\'re used to.</span>' : '') + '</div>', [
            ['Yes', 'Game.ClosePrompt();C.leaveTranscend(1);'], 'No'
        ]);
    } else {
        l('game').appendChild(transcendTransition);
        transcendAnimationStyle();
...
Game.registerHook('cps', cps => (C.calcDynamicTUEffs, cps));
...
Game.registerHook('check', function() {
    if (Game.getVeilDefense() > 1) Game.Win('Armored');
});
...
//puck
for (let i = 0; i < 3; i++) Game.wrinklers.push({
    id: parseInt(i) + 14,
    close: 0,
    sucked: 0,
    phase: 0,
...
C.puckMult = function(mult) {
    return 1 + ((mult ?? 1) * C.getEffects('Puck') * (Game.hasGod?.('scorn') ? 4 - Game.hasGod('scorn') : 0) * 0.01);
};
C.skruuiaT = 0;

C.mokalMult = function(mult) {
    return 1 + ((mult ?? 1) * C.getEffects('Mutation') * (Game.hasGod && Game.hasGod('mother') ? 4 - Game.hasGod('mother') : 0) * 0.01)
};
...
C.updateGarden = function() {
    ...
    mold.onHarvest = function() {
        C.gainChocolate(Math.floor(Math.random() * 2))
    };
...
C.onKaizo = function() {
    ...
    Game.Upgrades['A light in the dark'].descFunc = function() {
        return `Your Sparkling Wonder is <b>%%% ➡ %%%</b> more likely to save your Shimmering veil when it collapses. (Total: ${0.1 * (1 + C.getEffects(this.name)/100)})`
    };
...
C.updateYoupocalypse = function() {
    ...
    } else if (C.youWrath >= 2 && C.youWrath < 2.5) {
        Game.Win('Youpocalypse')
        C.youWrath = 2.5;
        C.crackCookie();
        Game.registerHook('click', function breakChunkClickFunc() {
            C.youWrath = Math.min(C.youWrath + 0.001, 3);
            if (Math.random() < 0.03) C.dropChunk(Math.floor(Math.random() * 10));
        });
    } else if (C.youWrath >= 2.5 && C.youWrath < 3) {
        C.youWrath = Math.min(C.youWrath + 0.0001, 3);
        if (Math.random() < 0.03 / 30) C.dropChunk(Math.floor(Math.random() * 10));
...
C.sacrificeClone = function() {
    if (Game.Objects['You'].amount > 0) {
        ...
        let message = choose(['The rest don\'t appear happy.', 'I\'m sure it\'s fine.', 'Everything goes on as normal.']);
...
PlaySound('snd/thud.mp3', 0.01);
PlaySound('snd/cookieBreak.mp3', 0.01);
...
C.crackCookie = function() {
    Game.CollectWrinklers();
    Crumbs.findObject('bigCookie').children.slice(2).forEach(o => o && (o.enabled = false));
    Crumbs.findObject('brokenCookie').enabled = true;
}

C.explodeCookie = function() {
    Game.registerHook('logic', C.cookieFallingAnimation);
    Game.CollectWrinklers();
    C.toggleBigCookie(0);
    C.doCookieFalling = true;
    Crumbs.findObject('bigCookie').children.slice(2).forEach(o => o && (o.enabled = false));
    C.brokenCookieObject.enabled = true;
    C.brokenCookieObject.children.slice(2, 12).forEach(x => x.behaviors.push(new Crumbs.behaviorInstance(C.chunkExplode, {
        cID: x.behaviors[0].cID,
        cNum: x.behaviors[0].cNum
    })));
    C.cookieFallingTimer = 0;
...
C.afterCookieExplode = function() {
    ...
    setTimeout(function() {
        ...
        Game.attachTooltip(puck, function() {
            return Game.crateTooltip(upgrade, 'transcend')
        }, 'this');

        C.transcendOffX = C.transcendOffXT = 0;
        C.transcendOffY = C.transcendOffYT = 0;

        Crumbs.spawn(C.ascendBlack);
        Crumbs.spawn(C.ascendRift);
...
Game.ResetWrinklers = function() {
    for (var i in Game.wrinklers) {
        Game.wrinklers[i] = {
            id: parseInt(i),
            close: parseInt(Game.wrinklers[i].type) == 2 ? 2 : 0,
            sucked: 0,
            phase: parseInt(Game.wrinklers[i].type) == 2 ? 2 : 0,
            x: 0,
            y: 0,
            r: 0,
            hurt: 0,
            hp: parseInt(Game.wrinklers[i].type) == 2 ? Game.wrinklers[i].hp : Game.wrinklerHP,
            selected: 0,
            type: parseInt(Game.wrinklers[i].type) == 2 ? 2 : 0,
            clicks: 0,
            ants: []
        };
    }
...
C.wrinklerAntDeath = function(w) {
    if (w.getChildren('ant').some(ant => ant.enabled)) {
        Game.Notify('Ants', 'Killing the wrinkler killed all ants on it.', [8, 1, icons]);
...
C.spawnAscendObject = function(o) {
    o.scope = 'background';
    let obj = Crumbs.spawn(o);
    obj.behaviors.unshift(new Crumbs.behaviorInstance(C.ascendObjectBehavior));
    return obj;
}
...
C.initClysmHUs = function() {
    ...
    //"something bad" path
    // Game.Upgrades['Aphids'].parents = [Game.Upgrades['Red ants']];
    // Game.Upgrades['Bug bites'].parents = [Game.Upgrades['Aphids']];
    // Game.Upgrades['Features'].parents = [Game.Upgrades['Bug bites']];
    Game.Upgrades['Aphids'].showIf = Game.Upgrades['Bug bites'].showIf = Game.Upgrades['Features'].showIf = function() {
        return false
    };
    Game.Upgrades['Some bees'].parents = [Game.Upgrades['Red ants']];
    ...
    Game.Upgrades['G. C. Organ'].parents = [Game.Upgrades['Barons']];
    ...
    Game.Upgrades['Industrialization'].parents = [Game.Upgrades['Captains']];
    ...
    Object.entries({
        ...
        Game.UpgradesById[x[0]].posX = x[1][0];
        Game.UpgradesById[x[0]].posY = x[1][1]
    });
}
C.initClysmHUs();
